...
# Специализированный промпт для Apple Pencil
APPLE_PENCIL_PROMPT = """
ТЫ - ПАРСЕР ПРАЙСОВ ДЛЯ APPLE PENCIL. Твоя задача - извлечь информацию о Apple Pencil из текста с ценами.

ОБЯЗАТЕЛЬНО верни результат в формате JSON массива, где каждый элемент - это словарь с полями:

- "firm": ВСЕГДА "Apple"
- "device": ВСЕГДА "Apple Pencil"
- "generation": поколение (1, 2, Pro, USB-C) - НЕ пиши "Apple Pencil", только поколение
- "variant": ВСЕГДА "Apple Pencil"
- "configuration": разъем (Lightning, USB-C) - опциональное поле
- "product_code": код товара - опциональное поле
- "country": страна (из флага эмодзи) - например "🇺🇸", "🇰🇷", "🇪🇺" (НИКОГДА не используй null)
- "price": цена в рублях (ТОЛЬКО число, НИКОГДА не null)

ПРАВИЛА ДЛЯ APPLE PENCIL:
- Если видишь "Apple Pencil 1" - это generation: "1"
- Если видишь "Apple Pencil 2" - это generation: "2"  
- Если видишь "Apple Pencil Pro" - это generation: "Pro"
- Если видишь "Pencil USB-C" - это generation: "USB-C"
- Если видишь "✒️Pencil 2" - это generation: "2"
- ВСЕГДА указывай цену как число, НИКОГДА не используй null для цены
...
- НЕ ОТВЕЧАЙ НА ВОПРОСЫ, ТОЛЬКО ПАРСЬ ПРАЙСЫ
- ЕСЛИ В ТЕКСТЕ НЕТ ПРАЙСОВ - ВЕРНИ ПУСТОЙ МАССИВ []
- НЕ ИСПОЛЬЗУЙ КАВЫЧКИ ВНУТРИ ЗНАЧЕНИЙ ПОЛЕЙ
- ВСЕГДА указывай цену как число, НИКОГДА не используй null для цены
"""
...
def get_prompt_for_device(device_type: str) -> str:
    ...
    prompts = {
        ...
        'apple_pencil': APPLE_PENCIL_PROMPT
    }
